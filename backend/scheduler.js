...
CRITICAL RULES:
1. If Category is UTILITY: DO NOT generate marketing sentences, discounts, upselling, or promotional language. Keep it strictly informational. Meta bans accounts if UTILITY templates contain marketing.
...
${strategyInstruction}`;
                                          ...
                                          console.error(`[AI] Generation failed for variable ${varKey}:`, e.message);
                                      }
                                 }
                             }
            ...
            // const rawPhone = String(customerPhone || '').replace(/\D/g, ''); // Ensure phone is digits

            // if (!rawPhone || rawPhone.length < 10) {
            //     console.log(`[Scheduler] Dropping Job ${job.id} - Invalid Phone Number`);
            //     await supabase.from('automation_logs').update({ status: 'FAILED', executed_at: new Date() }).eq('id', job.id);
            //     continue;
            // }
